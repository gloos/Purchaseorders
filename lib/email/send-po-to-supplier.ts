...
/**
 * Send a purchase order to the supplier via email with PDF attachment
 * @param poId The purchase order ID
 * @returns The Resend email ID
 */
export async function sendPOToSupplier(poId: string): Promise<string> {
  ...
  const organization = purchaseOrder.organization
...
${items
  .map(
    item =>
      `${item.description} - Qty: ${item.quantity} - Unit Price: ${currency} ${item.unitPrice.toFixed(2)} - Total: ${currency} ${item.total.toFixed(2)}`
  )
  .join('\n')}
...
`.trim()
  ...
  if (error) {
    ...
    throw new Error(error.message || 'Failed to send email to supplier')
  }
